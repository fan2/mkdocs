...
Software interrupts, or *SVCs* as they are now known, are generated by using the ARM instruction [SVC](https://developer.arm.com/documentation/ddi0602/latest/Base-Instructions/SVC--Supervisor-Call-?lang=en). This causes an exception to be taken, and forces the processor into Supervisor mode, which is privileged. A user program can request services from an operating system through such exception handling mechanisms.[^1]
...
[Register file of AArch64](./a64-regs.md)
[AArch64 Execution States](./a64-execution-state.md)
[AArch64 Exception Levels and Security States](./a64-exception-level.md)
