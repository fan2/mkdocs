...
假设我们执行 `readelf -SW test-gdb` 获取得到其中的 `.interp`、`.strtab`、`.shstrtab` 三个段的 Offset/Size 如下所示：
...
查阅相关资料可知，这 3 个 section 中的内容基本都是字符串。
...
可以使用 `readelf -p` 或 `objdump -j` 读取 section `.interp` 的内容：
...
其他两个 section `.strtab` 和 `.shstrtab` 读取情况如下：
...
除此之外，还可以综合使用 od, hexdump, strings 等工具 dump 文件指定范围的 bytearray/strings。
...
=== "od/hexdump range"
    ...
    $ od -j 0x001acf -N 0x0000fa -S 3 test-gdb
    $ hd -s 0x001acf -n 0x0000fa test-gdb
    ```
...
=== "head | tail | strings"
    ...
    $ head -c $((0x001898+0x000237)) test-gdb | tail -c $((0x000237)) | strings
